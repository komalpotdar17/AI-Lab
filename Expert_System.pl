...
% Book facts: name, author, genre, language, year, publisher, length, age_group
% Book facts: name, author, genre, language, year, publisher, length, age_group
book('To Kill a Mockingbird', 'Harper Lee, 1960, penguin, medium, adults).
...
find_books(Author, Genre, Language, Year, Publisher, Length, AgeGroup) :-
    findall(
        [Name, MatchScore],
        ( book(Name, A, G, L, Y, P, Len, AG),
          similarity_score([Author, Genre, Language, Year, Publisher, Length, roup], [A, G, L, Y, P, Len, AG], MatchScore)
        ),
        MatchedBooks),
